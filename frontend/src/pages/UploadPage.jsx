    ...
    <div className="max-w-[800px] mx-auto space-y-4">
      ...
      <AnimatePresence>
        {(insightsLoading || insights) && (
          ...
          >
            {/* Header */}
            <div className="flex items-center gap-2.5 px-4 py-3 border-b border-white/[0.06] bg-violet-500/[0.04]">
              <div className="w-8 h-8 rounded-[8px] bg-violet-500/[0.12] border border-violet-500/[0.18] flex items-center justify-center">
                {insightsLoading ? (
                  <RefreshCw className="w-[15px] h-[15px] text-violet-400 animate-spin" />
                ) : (
                  <Brain className="w-[15px] h-[15px] text-violet-400" />
                )}
              </div>
              <div className="flex-1">
                <h3 className="text-[14px] font-semibold text-white/90">AI Document Insights</h3>
                <p className="text-[11px] text-gray-400/70">
              ...
              {insightsLoading ? (
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className="h-3 rounded-full bg-white/[0.05] animate-pulse" style={{ width: `${85 - i * 15}%` }} />
                    ))}
                  </div>
                  ...
                  <div className="space-y-2 mt-3 pt-3 border-t border-white/[0.04]">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="h-3 rounded-full bg-white/[0.05] animate-pulse" style={{ width: `${70 - i * 12}%` }} />
                    ))}
                  </div>
                </div>
              ) : insights ? (
                <div className="space-y-3">
                  ...
                  {insights.totalPages != null && insights.totalChunks != null && (
                    <div className="flex items-center gap-3 mb-2">
                      <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-[6px] bg-white/[0.04] border border-white/[0.06]">
                        <BookOpen className="w-[13px] h-[13px] text-violet-400/70" />
                        <span className="text-[11px] text-gray-300">{insights.totalPages} pages</span>
                      </div>
                      <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-[6px] bg-white/[0.04] border border-white/[0.06]">
                        <Layers className="w-[13px] h-[13px] text-violet-400/70" />
                        <span className="text-[11px] text-gray-300">{insights.totalChunks} chunks</span>
                      </div>
                    </div>
                  )}

                  {/* Insight text */}
                  <div className="rounded-[10px] bg-violet-500/[0.04] border border-violet-500/[0.10] p-3.5">
                    <div className="flex items-start gap-2.5">
                      <Sparkles className="w-[16px] h-[16px] text-violet-400 shrink-0 mt-0.5" />
                      <p className="text-[13px] text-gray-300/90 leading-relaxed whitespace-pre-wrap">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-[7px] bg-violet-500/[0.10] border border-violet-500/[0.18] text-violet-400 hover:bg-violet-500/[0.18] hover:text-violet-300 text-[12px] font-medium transition-all duration-200"
                    >
...
function UploadQueueItem({ item, onRemove, onViewInsights, formatFileSize }) {
  const statusConfig = {
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        {item.status === 'completed' && item.documentId && onViewInsights && (
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 rounded-[6px] bg-violet-500/[0.12] border border-violet-500/[0.20] text-violet-400 hover:bg-violet-500/[0.20] hover:text-violet-300 text-[11px] font-medium transition-all duration-200"
            title="View AI Insights"
          >
            <Lightbulb className="w-[12px] h-[12px]" />
            Insights
          </button>
        )}
        <button
