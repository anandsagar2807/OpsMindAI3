...
export default function EnterpriseChatPage() {
  ...
  const loadChat = async (chatId) => {
    try {
      const token = await user?.getToken();
      const response = await chatAPI.getChat(chatId, token);
  ...
  const renameChat = async (chatId, newTitle) => {
    try {
      const token = await user?.getToken();
      await chatAPI.renameChat(chatId, newTitle, token);
      loadChatHistory();
  ...
  return (
    <EnterpriseLayout activeTab="chat">
      <div className="flex h-full">
        ...
        {/* Main Chat Area */}
        <div className="flex-1 flex flex-col bg-gradient-to-b from-slate-950/50 via-slate-950/30 to-indigo-950/20 backdrop-blur-sm">
          {/* Header */}
